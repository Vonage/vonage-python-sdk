...
@responses.activate
def test_create_call_phone_shaken():
    build_response(
        path, "POST", "https://api.nexmo.com/v1/calls", "create_call.json", 201
    )
    ncco = [Talk(text="Hello world")]
    to_phone = ToPhone(number="1234567890", dtmf_answer="1234", shaken="shaken-token")
    call = CreateCallRequest(
        ncco=ncco,
        to=[to_phone],
        from_={"number": "1234567890", "type": "phone"},
    ...
    body = json.loads(voice.http_client.last_request.body)
    assert body["to"][0]["number"] == "1234567890"
    assert body["to"][0]["dtmfAnswer"] == "1234"
    assert body["to"][0]["shaken"] == "shaken-token"
    assert type(response) == CreateCallResponse
