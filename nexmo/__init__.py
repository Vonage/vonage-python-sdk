...
class Client():
    ...
    def __init__(self, **kwargs):
        ...
        self.api_key = kwargs.get('key', None) or \
            os.environ.get('NEXMO_API_KEY')
        self.api_secret = kwargs.get('secret', None) or \
            os.environ.get('NEXMO_API_SECRET')
        self.signature_secret = kwargs.get('signature_secret', None) or \
            os.environ.get('NEXMO_SIGNATURE_SECRET', None)
        ...
        if 'app_name' in kwargs and 'app_version' in kwargs:
            user_agent += '/{0}/{1}'.format(kwargs['app_name'],
                                            kwargs['app_version'])
    ...
    def get_available_numbers(self, country_code, params=None, **kwargs):
        return self.get(self.host, '/number/search', dict(params or kwargs,
                        country=country_code))
    ...
    def get(self, host, request_uri, params={}):
        ...
        return self.parse(host, requests.get(uri, params=params,
                          headers=self.headers))

    def post(self, host, request_uri, params):
        uri = 'https://' + host + request_uri
        params = dict(params, api_key=self.api_key, api_secret=self.api_secret)
        return self.parse(host, requests.post(uri, data=params,
                          headers=self.headers))

    def put(self, host, request_uri, params):
        uri = 'https://' + host + request_uri
        params = dict(params, api_key=self.api_key, api_secret=self.api_secret)
        return self.parse(host, requests.put(uri, data=params,
                          headers=self.headers))

    def delete(self, host, request_uri):
        uri = 'https://' + host + request_uri
        params = dict(api_key=self.api_key, api_secret=self.api_secret)
        return self.parse(host, requests.delete(uri, params=params,
                          headers=self.headers))
    ...
    def __get(self, request_uri, params={}):
        ...
        return self.parse(self.api_host, requests.get(uri,
                          params=params, headers=self.__headers()))

    def __post(self, request_uri, params):
        uri = 'https://' + self.api_host + request_uri
        return self.parse(self.api_host, requests.post(uri, json=params,
                          headers=self.__headers()))

    def __put(self, request_uri, params):
        uri = 'https://' + self.api_host + request_uri
        return self.parse(self.api_host, requests.put(uri, json=params,
                          headers=self.__headers()))
